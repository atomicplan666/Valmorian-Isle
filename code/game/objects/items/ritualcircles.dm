...
/datum/outfit/job/roguetown/darksteelrite/pre_equip(mob/living/carbon/human/H, visualsOnly = FALSE)
	...
	if(!H.mind?.has_spell(/datum/action/cooldown/spell/mending))
...
/obj/structure/ritualcircle/baotha
	...
	var/baotharites = list("Rite of Armaments", "Joybringer", "Unholy Boon of Fertility")
...
/obj/structure/ritualcircle/baotha/attack_hand(mob/living/user)
	...
	switch(riteselection) // put ur rite selection here
		...
		if("Unholy Boon of Fertility")
			...
			icon_state = "baotha_active"
			user.apply_status_effect(/datum/status_effect/debuff/ritesexpended)
			baothafertility(blessing_target)
			spawn(120)
				icon_state = "baotha_chalky"

/obj/structure/ritualcircle/baotha/proc/baothafertility(mob/living/carbon/human/target)
	if(!target || QDELETED(target) || target.loc != loc)
		to_chat(usr, "Selected target is not on the rune!")
		return
